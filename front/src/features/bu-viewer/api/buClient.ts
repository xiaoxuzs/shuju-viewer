...
export async function fetchBuMatchProductXics(
  slug: string,
  matchId: number,
  request: BuProductXicBatchIn,
): Promise<BuProductXicBatchOut> {
  const { data } = await api.post<BuProductXicBatchOut>(
    `/datasets/${slug}/matches/${matchId}/product-xics`,
    request,
  );
  return data;
}
