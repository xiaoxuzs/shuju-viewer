    ...
    *,
    ...
    try:
        ...
        _update_job(
            ...
            stage="fingerprint",
            stage_label=_PHASE_LABELS["fingerprint"],
            stage_detail="Resolving dataset ingest root…",
            message="Resolving dataset ingest root…",
        ...
        if fp.file_count == 0:
            raise RuntimeError("No countable files under the selected path; import refused.")
        _update_job(
            job_id,
            progress=_PHASE_RANGES["fingerprint"][1],
            stage_detail=(
                f"Fingerprint done: {fp.file_count} files in {fp.elapsed_seconds:.3f}s, digest={fp.fingerprint}"
            ),
        )
        _slice("fingerprint_job_updates_s")
        ...
        try:
            ...
            raise RuntimeError(
                "Dataset fingerprint conflicts with an existing record (concurrent import rolled back). "
                "Retry later or delete the conflicting dataset."
            ) from exc
