"""Protein list and detail endpoints for a given cutoff; reads the universal schema."""
...
def list_proteins(
    ...
    """List proteins with pagination; optional fuzzy search on name/description. `total` is the filtered row count.

    Cutoff is enforced via ``EXISTS (identification_matches WHERE source_cutoff = :cutoff)``
    on the protein → proteoform → match chain; only proteins with IDs under that cutoff appear.
    """
...
def get_protein(
    ...
    """Single protein detail; path ``protein_id`` is ``proteins.protein_id`` (PK).

    Cutoff: require at least one ``identification_matches`` row for this protein under the current cutoff;
    nested proteoforms are filtered the same way.
    """
    dataset = require_dataset(session, slug)
    require_cutoff(cutoff)
