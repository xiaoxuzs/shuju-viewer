...
参见：`数据导入模块.md`、`RAW原始文件模块.md`、`Bottom-Up模块.md`、`Top-Down模块.md`。
...
相关测试入口以 `测试模块.md` 的完整分层为准；本模块重点参考 `back\tests\test_import_planner.py`、`back\tests\test_import_planner_raw.py`、`back\tests\test_raw_mzml_mapping.py`。
