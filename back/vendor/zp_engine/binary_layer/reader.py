...
from .v3_limits import V3_ARRAY_READ_LIMITS
...
class ZpReader:
    def __init__(
        ...
        file_path: str | Path,
        *,
        v2_limits: ZpV2ArrayReadLimits | None = None,
        v3_limits: ZpV2ArrayReadLimits | None = None,
    ) -> None:
        self.file_path = Path(file_path)
        self._v2_arrays_reader = ZpV2ArraysReader(v2_limits)
        self._v3_arrays_reader = ZpV3ArraysReader(
            V3_ARRAY_READ_LIMITS if v3_limits is None else v3_limits
        )
        ...
        self._v2_arrays_directory_cache: tuple[
            _FileFingerprint,
            V2ArraysDirectory | V3ArraysDirectory,
            bool,
        ] | None = None
    ...
    def _read_v2_directory(
        ...
        for position, (raw_entry, expected_name) in enumerate(zip(parsed, BLOCK_NAMES)):
            ...
            expected_encoding = (
                V3_ARRAYS_ENCODING
                if header.version == ZP_VERSION_V3 and expected_name == "arrays"
                else "zp-arrays-v2"
                if expected_name == "arrays"
                else V3_EXTENSIONS_ENCODING
                if header.version == ZP_VERSION_V3 and expected_name == "extensions"
                else "utf-8-json"
            )
    ...
    def _read_json_block(
        ...
        if header.version == ZP_VERSION_V3 and block_name == "extensions":
            return decode_v3_extensions(payload)
        try:
    ...
    def _v2_arrays_directory(
        ...
        *,
    ...
    ) -> tuple[BlockDirectoryEntry, V2ArraysDirectory | V3ArraysDirectory]:
        ...
        arrays_reader = (
            self._v3_arrays_reader
            if header.version == ZP_VERSION_V3
            else self._v2_arrays_reader
        )
        directory = arrays_reader.read_directory(
            stream,
    ...
    def _read_v2_arrays_by_ids(
        ...
        decoded = {
            array_id: arrays_reader.read_array(
                stream,
                block_offset=arrays_entry.offset,
                directory=directory,
                array_id=array_id,
    ...
    def _read_all_v2_arrays(
        ...
        budget = arrays_reader.limits.max_decoded_memory
