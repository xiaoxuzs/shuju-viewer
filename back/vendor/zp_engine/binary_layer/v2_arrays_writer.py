...
def _numeric_buffer(array: ArrayBlock, *, validate_types: bool) -> np.ndarray:
    if isinstance(array.values, NativeFloat64Array):
        values = array.values
        if (
            ...
            _fail(
                "INVALID_NATIVE_ARRAY",
                "native float64 arrays must be one-dimensional, contiguous, and read-only",
...
def prepare_v2_arrays_layout(
    ...
    *,
    ...
    for position, array in enumerate(array_view):
        ...
        if not isinstance(array.values, (list, np.ndarray)):
            _fail(
                "INVALID_ARRAY_VALUE",
                "values must be a list or a one-dimensional float64 NumPy array",
                f"{location}.values",
                actual=type(array.values).__name__,
            )
        _check_limit(
    ...
    for array in sorted_arrays:
        trusted_checksum = (
            array.values.native_sha256
            if isinstance(array.values, NativeFloat64Array)
            else None
        )
        if trusted_checksum is None:
            checksum = hashlib.sha256()
            for chunk in _encoded_chunks(array, validate_types=True):
                checksum.update(chunk)
            checksum_text = checksum.hexdigest()
        else:
            _numeric_buffer(array, validate_types=True)
            checksum_text = trusted_checksum
...
def write_v2_arrays_block(stream: BinaryIO, layout: V2ArraysLayout) -> tuple[int, str]:
    ...
    for array, entry in zip(layout.arrays, layout.entries):
        trusted = isinstance(array.values, NativeFloat64Array)
        array_digest = None if trusted else hashlib.sha256()
        array_written = 0
        ...
        checksum_changed = (
            array_digest is not None
            and array_digest.hexdigest() != entry.checksum
        )
        if array_written != entry.byte_length or checksum_changed:
            _fail(
