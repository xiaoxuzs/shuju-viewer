...
from .real_mzml import (
    build_mzml_candidate,
    format_mzml_admission_issues,
    validate_mzml_candidate,
)
...
class RealDiaResultTool(BaseBlockTool):
    ...
    def build_blocks(self, context: PipelineContext) -> None:
        ...
        conversion_options = context.metadata.get("conversion_options")
        document = parse_mzml(
            bundle.spectrum_source,
            worker_threads=getattr(conversion_options, "worker_threads", 6),
        )
        dia_features = []
        ...
        if not admission.accepted:
            raise MzmlAdmissionError(
                format_mzml_admission_issues(admission.issues)
            )
        ...
        adapter_report: DiaResultAdapterReport = self.adapter.read(
            ...
            compact_source_tables=context.metadata.get("format_version") == 3,
        )
