...
class HashInputStep(BasePipelineStep):
    ...
    def run(self, context: PipelineContext) -> None:
        ...
        precomputed_digest = context.metadata.get("_precomputed_input_sha256")
        precomputed_hashes = context.metadata.get("_precomputed_source_file_hashes")
        if (
            isinstance(precomputed_digest, str)
            and len(precomputed_digest) == 64
            and isinstance(precomputed_hashes, dict)
            and all(
                isinstance(key, str)
            ...
            context.metadata["input_sha256"] = precomputed_digest
            context.metadata["source_file_hashes"] = dict(precomputed_hashes)
            return
    ...
    def run(self, context: PipelineContext) -> None:
        ...
        if format_version is None:
            ...
            written = self.writer.write(
                ...
                v3_limits=context.metadata.get("v3_array_write_limits"),
                worker_threads=getattr(
                    context.metadata.get("conversion_options"),
                    "worker_threads",
                    6,
                ),
                v3_array_compression=getattr(
                    context.metadata.get("conversion_options"),
...
class ZpValidateStep(BasePipelineStep):
    ...
    def run(self, context: PipelineContext) -> None:
        ...
        if (
            ...
            and context.metadata.get("integrated_v3_validation") is True
            and isinstance(context.artifacts.get("zp_writer_metrics"), dict)
        ):
            result = validate_quick(Path(path))
            cached_extensions = list(context.blocks.extensions)
            ...
            result.mode = "deep"
            result.metrics["validation_strategy"] = (
                "integrated-write-validation-plus-full-checksum-readback"
            )
            context.blocks = BlockCollection()
            ...
            if not result.valid:
                ...
                raise ZpValidationError(
                    f"Generated .zp failed integrated validation: {codes}"
                )
            return
