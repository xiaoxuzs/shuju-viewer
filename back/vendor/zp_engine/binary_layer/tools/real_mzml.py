    ...
    def build_blocks(self, context: PipelineContext) -> None:
        ...
        if not isinstance(options, ConversionOptions):
            raise MzmlParseError(
                ...
                "conversion_options must be a ConversionOptions value",
                "context.metadata.conversion_options",
            )
        document = parse_mzml(
            context.source_profile.input_files[0],
        ...
        if not admission.accepted:
            ...
            raise MzmlAdmissionError(format_mzml_admission_issues(admission.issues))
        ...
        if source_file_label is None:
            source_file_label = document.source_path.name
        ...
        candidate = build_mzml_candidate(
            ...
            compact_arrays=context.metadata.get("format_version") == 3,
        )
...
def format_mzml_admission_issues(
    ...
    *,
    ...
    details = "; ".join(
        f"{getattr(item, 'code')} at {getattr(item, 'location')}: "
        f"{getattr(item, 'message')}"
        for item in visible
    )
    omitted = len(issues) - len(visible)
    return (
        f"{details}; ... {omitted} additional admission issues omitted"
        if omitted
        else details
    )
    ...
    *,
        ...
        arrays.extend(
            (
                ArrayBlock(
                    ...
                    "mz",
                    "float64",
                    _normalized_values(item.mz_values, compact=compact_arrays),
                ),
                ArrayBlock(
                    intensity_array_id,
                    "intensity",
                    "float64",
                    _normalized_values(
                        item.intensity_values,
                        compact=compact_arrays,
                    ),
                ),
            )
        )
    ...
    for position, item in enumerate(admitted_chromatograms, 1):
        ...
        if (
            ...
            or len(item.time_values_seconds) == 0
            or len(item.intensity_values) == 0
...
def validate_mzml_candidate(candidate: BlockCollection) -> None:
    ...
    for chromatogram in candidate.chromatograms:
        ...
        if (
            len(time_array.values) == 0
            or len(intensity_array.values) == 0
            or len(time_array.values) != len(intensity_array.values)
        ):
...
def _normalized_values(
    ...
    *,
    compact: bool,
) -> NormalizedFloat64List | np.ndarray:
    if compact:
        if isinstance(values, NativeFloat64Array):
            return values
        return np.ascontiguousarray(values, dtype="<f8")
    return NormalizedFloat64List(values)  # type: ignore[arg-type]
