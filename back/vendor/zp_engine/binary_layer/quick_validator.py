...
from .v3_extensions import V3_EXTENSIONS_ENCODING
from .v3_arrays import V3_ARRAYS_ENCODING
...
def _parse_directory(
    ...
    if not isinstance(parsed, list):
        ...
        return None
    if version in {ZP_VERSION_V2, ZP_VERSION_V3}:
        try:
...
def _validate_directory_entries(
    ...
    expected_encodings = {
        name: (
            ...
            else V3_ARRAYS_ENCODING
            if version == ZP_VERSION_V3 and name == "arrays"
            else "zp-arrays-v2"
            if name == "arrays"
            else V3_EXTENSIONS_ENCODING
            if version == ZP_VERSION_V3 and name == "extensions"
            else "utf-8-json"
        )
