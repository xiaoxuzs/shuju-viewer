...
from .dia_resource_limits import DIA_V2_VALIDATION_LIMITS
...
from .source_adapters import build_default_source_adapter_registry
...
def convert_source_to_zp(
    ...
    *,
    ...
    source_before, source_file_hashes, source_fingerprints = (
        _source_identity_details(source, profile)
    )
    source_adapters = build_default_source_adapter_registry()
    source_adapter = source_adapters.get(profile.source_type)
    plan = PlanBuilder(source_adapters).build(profile)
    ...
    context = PipelineContext(
        ...
        metadata={
            ...
            "release_blocks_after_write": format_version != 3,
            "integrated_v3_validation": format_version == 3,
            "_precomputed_input_sha256": source_before.sha256,
            "_precomputed_source_file_hashes": source_file_hashes,
        },
    )
    source_adapter.configure_context(context)
    ...
    try:
        ...
        if _source_fingerprints(source, profile) != source_fingerprints:
            raise SourceConversionError(
                "SOURCE_CHANGED_DURING_CONVERSION",
                "Source file identity, size, or timestamps changed during conversion",
            )
    ...
    output_sha256 = (
        ...
        if isinstance(validation.file_sha256, str)
        else _sha256(target)
    )
...
def _source_identity(path: Path, profile: SourceProfile) -> SourceFileIdentity:
    identity, _hashes, _fingerprints = _source_identity_details(path, profile)
    return identity


def _source_identity_details(
    path: Path,
    ...
    if not profile.identity_files:
        ...
        stat = path.stat()
        if (
            stat.st_size != identity.file_size
            or stat.st_mtime_ns != identity.mtime_ns
        ):
            raise SourceConversionError(
                "SOURCE_CHANGED_DURING_FINGERPRINT",
                f"Source changed while recording identity: {path}",
            )
        label = profile.relative_label(path)
        return (
            identity,
            {label: identity.sha256},
            (_stat_fingerprint(path, stat),),
        )
    digest = hashlib.sha256()
    total_size = 0
    mtimes: list[int] = []
    source_hashes: dict[str, str] = {}
    fingerprints: list[tuple[str, int, int, int, int, int]] = []
    for item in sorted(
        ...
        label_text = profile.relative_label(item)
        source_hashes[label_text] = item_digest.hexdigest()
        fingerprints.append(_stat_fingerprint(item, after))
    return (
        SourceFileIdentity(total_size, digest.hexdigest(), max(mtimes)),
        source_hashes,
        tuple(fingerprints),
    )
...
def _source_fingerprints(
    ...
    paths = profile.identity_files or (path,)
    try:
        return tuple(
            _stat_fingerprint(item, item.stat())
            for item in sorted(
                paths,
                key=lambda candidate: profile.relative_label(candidate).encode("utf-8"),
            )
        ...
        raise SourceConversionError(
            "SOURCE_NOT_READABLE",
            "Source identity could not be rechecked after conversion",
        ) from exc
