...
_DIANN_SPECS_BY_NAME = {
    item.source_name: item for item in DIANN_COLUMN_SPECS
}
_IDENTIFICATION_QUANT_COLUMN_ORDER = tuple(
    name for name in DIANN_COLUMN_NAMES if name in _IDENTIFICATION_QUANT_COLUMNS
)
_GROUP_QUANT_COLUMN_ORDER = tuple(
    name for name in DIANN_COLUMN_NAMES if name in _GROUP_QUANT_COLUMNS
)
...
class DiaResultAdapter:
    ...
    def read(
        ...
        *,
        ...
        compact_source_tables: bool = False,
        ...
        for batch in parquet.iter_batches(batch_size=self.batch_size):
            ...
            selection = pc.and_kleene(
                pc.less(batch.column(names.index("Q.Value")), Q_VALUE_CUTOFF),
                pc.equal(batch.column(names.index("Decoy")), 0),
            )
            selection = pc.fill_null(selection, False)
            selected_batch = batch.filter(selection)
            selected_rows = selected_batch.to_pylist()
            if unknown:
                for row in selected_rows:
                    for name in unknown:
                        row[name] = _canonical_source_value(row.get(name))
            if compact_source_tables:
                stored_columns: object = batch
            else:
                stored_columns = {
                    name: [
                        _canonical_source_value(value)
                        for value in batch.column(position).to_pylist()
                    ]
            ...
            for row in selected_rows:
                ...
                ident_measurements = _measurement_values(
                    typed,
                    _IDENTIFICATION_QUANT_COLUMN_ORDER,
                )
                ...
                if group_id is not None:
                    group_measurements = _measurement_values(
                        typed,
                        _GROUP_QUANT_COLUMN_ORDER,
                    )
...
def _typed_row(row: dict[str, Any], present: tuple[str, ...]) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for name in present:
        spec = _DIANN_SPECS_BY_NAME[name]
        value = row.get(name)
...
def _measurement_values(
    ...
    for name in columns:
        spec = _DIANN_SPECS_BY_NAME[name]
        if spec.logical_field not in typed:
            continue
        value = typed[spec.logical_field]
        if value is None:
            result[spec.logical_field] = None
            continue
        ...
        result[spec.logical_field] = value
    return result
