...
from .v3_limits import V3_VALIDATION_LIMITS
...
class ZpValidator:
    ...
    v3_limits: ZpV2ValidationLimits = V3_VALIDATION_LIMITS
            ...
            with path.open("rb") as stream:
                ...
                if version in {ZP_VERSION_V2, ZP_VERSION_V3}:
                    if magic != ZP_MAGIC or endianness != ZP_ENDIANNESS_LITTLE:
                        return self._result(path, version, issues, checked_blocks)
                    limits = self.v3_limits if version == ZP_VERSION_V3 else self.v2_limits
                    validator = ZpV2Validator(limits)
