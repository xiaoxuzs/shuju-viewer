...
from .exceptions import MzmlSchemaError, ZpReadError, ZpV2ArrayReadError
...
from .v3_arrays import V3_ARRAYS_ENCODING, V3ArrayReadLimits, ZpV3ArraysReader
...
def _validate_json_schemas(blocks: dict[str, object], state: _ValidationState) -> None:
    ...
    if _field_set_is_exact(meta, _GLOBAL_FIELDS, "global_meta", state, "global_meta"):
        ...
        if meta["format_version"] != state.version or not _plain_int(meta["format_version"]):
            state.add(
                "FORMAT_VERSION_MISMATCH",
                "global_meta.format_version must match the Header version",
...
def _read_top_directory(
    ...
    *,
    ...
    for position, expected_name in enumerate(BLOCK_NAMES):
        ...
        expected_encoding = (
            V3_ARRAYS_ENCODING
            if state.version == ZP_VERSION_V3 and expected_name == "arrays"
            else "zp-arrays-v2"
            if expected_name == "arrays"
            else V3_EXTENSIONS_ENCODING
            if state.version == ZP_VERSION_V3 and expected_name == "extensions"
            else "utf-8-json"
        )
...
def _read_json_blocks(
    ...
    json_block_names = (
        tuple(name for name in _JSON_BLOCK_NAMES if name != "extensions")
        if state.version == ZP_VERSION_V3
        else _JSON_BLOCK_NAMES
    )
    for block_name in json_block_names:
        wall_started = time.perf_counter()
        cpu_started = time.process_time()
...
def _read_v3_extensions(
    state: _ValidationState,
    entries: tuple[BlockDirectoryEntry, ...],
) -> list[dict[str, object]]:
    entry = next(item for item in entries if item.block_name == "extensions")
    state.check_limit(
        "VALIDATION_WORK_MEMORY_EXCEEDED",
        entry.length,
        state.limits.max_work_memory,
    ...
    state.stream.seek(entry.offset)
    payload = state.read_exact(entry.length, "TRUNCATED_BLOCK", "extensions")
    state.checked_blocks += 1
    actual_checksum = hashlib.sha256(payload).hexdigest()
    if actual_checksum != entry.checksum:
        state.add(
    ...
    try:
        return decode_v3_extensions(
            payload,
            materialize_column_tables=False,
        )
    except ZpReadError as exc:
        state.stop(
            "INVALID_V3_EXTENSIONS",
            str(exc),
            "extensions",
        )
...
def _validate_v3_arrays_region(
    state: _ValidationState, entry: BlockDirectoryEntry
) -> tuple[_ArrayMeta, ...]:
    limits = state.limits
    state.metrics["arrays_size"] = entry.length
    read_limits = V3ArrayReadLimits(
        max_arrays_block_length=limits.max_arrays_block_length,
        max_directory_length=limits.max_array_directory_length,
        ...
        max_decoded_memory=limits.max_work_memory,
    )
    reader = ZpV3ArraysReader(read_limits)
    try:
    ...
    except ZpV2ArrayReadError as exc:
        state.stop(
            exc.code,
            exc.message,
            exc.location,
    ...
    state.metrics["arrays_payload_length"] = directory.payload_length
    state.metrics["entry_count"] = len(metas)
    state.metrics["numeric_value_count"] = sum(item.value_count for item in metas)
    state.metrics["payload_access_backend"] = "zstd-chunked-read"
    state.metrics["compressed_chunk_count"] = len(directory.chunks)
    ...
    while remaining:
        ...
        stored = state.read_exact(
            requested,
            "ARRAY_PAYLOAD_OUT_OF_BOUNDS",
            "arrays",
        )
        whole_hash.update(stored)
        remaining -= len(stored)
    actual_whole_checksum = whole_hash.hexdigest()
    if actual_whole_checksum != entry.checksum:
        state.add(
            "BLOCK_CHECKSUM_MISMATCH",
            "top-level arrays checksum does not match raw stored bytes",
            "arrays",
            actual=actual_whole_checksum,
        )
    ...
    array_seen = 0
    numeric_issue_recorded = False

    def finish_empty_or_complete() -> None:
        nonlocal meta_index, array_digest, array_seen, numeric_issue_recorded
        while meta_index < len(metas):
            meta = metas[meta_index]
            if array_seen != meta.byte_length:
                break
            actual = array_digest.hexdigest()
            if actual != meta.checksum:
                per_array_issues.append(
                    _issue(
                        "ARRAY_CHECKSUM_MISMATCH",
                        "array payload checksum does not match its entry",
                        f"arrays[{meta.array_id}].checksum",
                        actual=actual,
                    )
                )
            meta_index += 1
    ...
    for chunk in directory.chunks:
        ...
        state.metrics["max_single_payload_read"] = max(
            int(state.metrics["max_single_payload_read"]),
            chunk.stored_length,
        )
        cursor = 0
        while cursor < len(raw):
            if meta_index >= len(metas):
                state.stop(
                    "INVALID_ARRAY_PAYLOAD_LENGTH",
                    "decoded chunks exceed the logical arrays payload",
                    "arrays.payload_length",
                )
            meta = metas[meta_index]
            take = min(meta.byte_length - array_seen, len(raw) - cursor)
            view = memoryview(raw)[cursor : cursor + take]
            array_digest.update(view)
            if not numeric_issue_recorded:
                numeric_issue_recorded = _check_numeric_values(
                    meta,
                    view,
                    array_seen // 8,
                    numeric_issues,
    ...
    if meta_index != len(metas):
        state.stop(
            "INVALID_ARRAY_PAYLOAD_LENGTH",
            "decoded chunks do not cover the logical arrays payload",
            "arrays.payload_length",
            actual=meta_index,
            limit=len(metas),
        )
    ...
    def validate_stream(
        ...
        *,
        ...
        try:
            ...
            if version not in {ZP_VERSION_V2, ZP_VERSION_V3}:
                state.stop("UNSUPPORTED_VERSION", "binary validator received an unsupported version", "header.version", actual=version)
            if flags != 0:
            ...
            if version == ZP_VERSION_V3:
                blocks["extensions"] = _read_v3_extensions(state, entries)
            _validate_json_schemas(blocks, state)
            ...
            arrays = (
                _validate_v3_arrays_region(state, arrays_entry)
                if version == ZP_VERSION_V3
                else _validate_arrays_region(state, arrays_entry)
            )
