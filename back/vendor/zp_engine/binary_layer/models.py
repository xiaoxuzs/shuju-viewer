...
class ConversionOptions:
    ...
    v3_array_compression: Literal["zstd", "raw"] = "zstd"
    ...
    def __post_init__(self) -> None:
        ...
        if (
            type(self.worker_threads) is not int
            or not 1 <= self.worker_threads <= 32
        ):
            raise ValueError("worker_threads must be an integer from 1 to 32")
        if self.v3_array_compression not in {"zstd", "raw"}:
            raise ValueError("v3_array_compression must be 'zstd' or 'raw'")
